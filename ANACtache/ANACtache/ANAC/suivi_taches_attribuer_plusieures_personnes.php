<?php
 ...
 { 
   ...
   $output = '';
      ...
      $query = " SELECT * FROM  attributeur_tache WHERE nomattributeur!='' AND personnelanac='OUI' GROUP BY nomattributeur  ORDER BY nomattributeur";
...
<center><i style="color:red;font-weight:bold">Les champs marqués d'un astérisques(*) sont obligatoire </i></center>
      ...
      <!--quand on clique sur le btn planifier=enregistrer on stok dans la table suivi_tache,et on envoi un email automatiquema o user RMO attributeur choisi-->

      <form class="form-horizontal" method="POST" action="traitement_suivi_tache_plusiere_Attributaire.php" enctype="multipart/form-data">
...
while ($data2 = mysqli_fetch_assoc($requete_statut_tache_plusiuers_attributer)) {
        ...
        data-idstatuttache="<?php $data2['idstatuttache'];?>"     data-nomstatuttache="<?php echo $data2['nomstatuttache'];?>"
  ...
  <div class="row" style="background-color:#03224c;color:white" ><!--COULEUR DE TOUTES LA PAGE EN FOND BLEU-->
    ...
    <div class="form-group has-success has-feedback">
          ...
          <label class="col-sm-1 control-label"><font color="white">Date courrier/mail:
              ...
              <div class="col-sm-3">
              <input type="date" title="Date courrier/mail reçus" required="required" value="<?php echo $date=date('Y-m-d' );?>"  name="dateenrgecouranac"  style="color:black;font-weight:bold" class="form-control">
...
<label class="col-sm-1 control-label"><font color="white">Date Attribution:
                                                <i style="color:red;font-weight:bold">*</i></font></label>
                                                    ...
                                                    <input type="date" title="Date Attribution de la tâche" required="required" value="<?php echo $date=date('Y-m-d' );?>"  readonly name="dateattribution"  style="color:black;font-weight:bold" class="form-control">
